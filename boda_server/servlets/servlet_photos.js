...
const { consulta } = require('../bd/wrapperBD.js');
...
const storage = upload.diskStorage({
    ...
    filename: (req, err, cb) => {
        ...
        // Generate a unique filename combining table/challenge IDs and timestamp
        cb(null, `${mesaId}_${retoId}-${Date.now()}-${req.file.originalname}`);
    }
});

const photoUploadMiddleware = upload.single('foto');
